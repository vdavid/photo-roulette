...
test.describe('Full 4-player game', () => {
	...
	test('plays a complete game with correct scoring', async ({ browser }, testInfo) => {
		...
		try {
			...
			await test.step('Host creates game', async () => {
				// Navigate directly to Photo Roulette landing page
			...
			await test.step('Other players join', async () => {
				for (const player of [player2, player3, player4]) {
					...
					await player.page.evaluate((resultDisplayMs) => {
						(
							window as unknown as { __TEST_RESULT_DISPLAY_MS__: number }
						).__TEST_RESULT_DISPLAY_MS__ = resultDisplayMs;
					}, RESULT_DISPLAY_MS);
